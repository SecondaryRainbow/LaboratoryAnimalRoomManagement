  ...
  <div style="margin-top: 60px;margin-left:330px;width: 300px;height: 500px;border: 0px solid red;">
    <el-form ref="ruleForm" :model="ruleForm" :rules="rules" class="demo-ruleForm" label-width="100px">
      <el-form-item label="房间编号">
        <el-input v-model="ruleForm.animalRoomId" disabled="true"></el-input>
      </el-form-item>
      <el-form-item label="房间总容量">
        <el-input v-model="ruleForm.animalRoomTotalCapacity" disabled="true"></el-input>
      </el-form-item>
      <el-form-item label="房间当前容量">
        <el-input v-model="ruleForm.animalRoomCurrentCapacity" disabled="true"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const _this = this
          if (_this.ruleForm.animalRoomAvailability === 'N') {
            this.$confirm('将【房间可用性】改为【不可用】会将该房间中的动物组全部清出房间，确认继续吗？', '警告', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              axios.put('http://localhost:8181/animalRoomInfo/update', _this.ruleForm).then(function (resp) {
                if (resp.data.code === 0) {
                  _this.$alert(_this.ruleForm.animalRoomId + '修改成功', '', {
                    confirmButtonText: '确定',
                    callback: action => {
                  ...
                  _this.$alert(_this.ruleForm.animalRoomId + '修改失败', '', {
                    confirmButtonText: '确定',
                  });
                }
              })
  ...
  created() {
    const _this = this
    axios.get('http://localhost:8181/animalRoomInfo/findById/' + _this.$route.query.animalRoomId).then(function (resp) {
