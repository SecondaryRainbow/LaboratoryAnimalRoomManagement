<template>
  <el-container class="home_container">
    <el-header class="home_header">
      <div class="home_title">LARM实验动物房管理系统-移送申请者</div>
    ...
    <el-container>
      ...
      <el-aside class="home_aside" width="200px">
        ...
        <el-menu router>
          <el-submenu index="1">
            <template slot="title"><i class="el-icon-s-platform"></i>用户模块</template>
            <el-menu-item index="/systemUserAdd" :class="$route.path=='/systemUserAdd'?'is-active':''">
              <i class="el-icon-folder-add"></i>添加用户
            </el-menu-item>
            <el-menu-item index="/systemUserManagement" :class="$route.path=='/systemUserManagement'?'is-active':''">
...
export default{
  methods: {
    logout(){
      ...
      this.$confirm('注销登录吗?', '提示', {
        ...
        localStorage.removeItem('animalTransportPersonnel')
  ...
  created() {
    let admin = JSON.parse(window.localStorage.getItem('animalTransportPersonnel'))
    this.admin = admin
  }
}
</script>
